...
domain_2_net.load_state_dict(torch.load('/data2/yangbo/NS_pipe_DD_reorder/train_UZ_40400/part23_U/part_model-500000.pt', 
                                         map_location=device, weights_only=False)['model_state_dict'])
domain_2_net.eval()

domain_3_net = MIONetCartesianProd_3(
    [6 , 512],
    [40, 512, 512, 512],
    [1 , 512, 512, 512],
    [2 , 512, 512, 512],
...
domain_3_net.load_state_dict(torch.load('/data2/yangbo/NS_pipe_DD_reorder/train_UZ_40400/part4/part4_model-500000.pt', 
                                         map_location=device, weights_only=False)['model_state_dict'])
domain_3_net.eval()
...
# domain_2 trunk
domain_3_trunk = np.loadtxt("/data2/project_share/yangbo/NS_pipe_DD_reorder/trunk_UZ/trunk_part2.txt", dtype=np.float32)
domain_3_trunk = torch.from_numpy(domain_3_trunk).to(device)
...
domain_1_branch_2_skew = np.loadtxt("/data2/project_share/yangbo/NS_pipe_DD_reorder/data_U_skewing/Dirichlet_inlet.txt")
...
dx1 = np.concatenate([dx1, dx1_01])
dy1 = np.concatenate([dy1, dy1_01])
dx2 = np.concatenate([dx2, dx2_01])
dy2 = np.concatenate([dy2, dy2_01])
dx3 = np.concatenate([dx3, dx3_01])
dy3 = np.concatenate([dy3, dy3_01])
...
domain_1_branch_2 = torch.cat([domain_1_branch_2_skew, domain_1_branch_2_01], dim=0)
...
domain_3_branch_1 = np.column_stack([dy3, 1 / dy3, 1 / dy3**2, dx3, 1 / dx3, 1 / dx3**2])
domain_3_branch_1 = torch.from_numpy(domain_3_branch_1.astype(np.float32)).to(device)
domain_3_branch_3 = dx2 / dx3
domain_3_branch_3 = domain_3_branch_3.reshape(-1, 1)
domain_3_branch_3 = torch.from_numpy(domain_3_branch_3.astype(np.float32)).to(device)

###########################################################################################################################################
# interface coordinate
coord_part1 = torch.from_numpy(np.ones([40 * test_num, 2]).astype(np.float32)).to(device)
coord_part3 = torch.from_numpy(np.ones([40 * test_num, 2]).astype(np.float32)).to(device)
...
x_interval = 1 / (40 - 1)
coord_part3_reshaped = coord_part3.view(-1, 40, 2)
coord_part3_reshaped[:, :, 0] = torch.arange(40, device=device) * x_interval
coord_part3 = coord_part3_reshaped.view(-1, 2)
part3_y = (dx3 - dx2) / dx3 
part3_y = torch.from_numpy(part3_y.astype(np.float32)).to(device)
part3_y_expanded = part3_y.unsqueeze(1).repeat(1, 40)
coord_part3[:, 1] = part3_y_expanded.flatten()
...
filename_domain_1 = '/data2/yangbo/NS_pipe_DD_reorder/train_UZ_40400/DD_U/batch/both/part1.txt'
filename_domain_2 = '/data2/yangbo/NS_pipe_DD_reorder/train_UZ_40400/DD_U/batch/both/part2.txt'
filename_domain_3 = '/data2/yangbo/NS_pipe_DD_reorder/train_UZ_40400/DD_U/batch/both/part3.txt'
...
with open(filename_domain_1, 'w') as f_1, open(filename_domain_2, 'w') as f_2, open(filename_domain_3, 'w') as f_3:
    ...
    while  iteration < Iter:
        ...
        part2_branch3 = domain_3_net((domain_3_branch_1, domain_3_branch_2, domain_3_branch_3, coord_part3)) # part3中计算
        domain_2_branch_3 = part2_branch3[row_indices, col_indices].view(test_num, BC_num)
        del part2_branch3
...
#############################################################################
    ...
    # compute domain_3
    domain_3_input = (domain_3_branch_1, domain_3_branch_2, domain_3_branch_3, domain_3_trunk)
    domain_3_result = domain_3_net(domain_3_input)
